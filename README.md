...
3. **학사일정** 넣기: 구글 캘린더에서 받은 `.ics` 파일, 또는 시트의 "날짜 | 일정" 목록 붙여넣기
4. 자동 분류 결과 확인 → 결과 보기
...
| `6(1)` | 그날 6교시와 1교시를 맞바꿈 (창체 시간을 당겨 쓰는 경우) |
| `(1,2학년) 중간고사`, `2학년 수련회` | 해당 학년만 |
...
web/src/engine/   계산 엔진 (화면과 분리된 순수 함수, 테스트 포함)
  ...
  suggest.ts        보완 제안 탐색
  importTimetable.ts, ics.ts, eventList.ts   가져오기
